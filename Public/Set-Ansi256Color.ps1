...
function Get-Ansi256ColorChart {
    ...
    if ($ShowStandard) {
        ...
        for ($i = 0; $i -lt 16; $i++) {
            $foregroundColor = if ($i -lt 8) { 255 } else { 0 }
            $colorText = Set-Ansi256Color -Text (" {0:D3} " -f $i) -BackgroundColor $i -ForegroundColor $foregroundColor
    ...
    for ($r = 0; $r -lt 6; $r++) {
        ...
        for ($g = 0; $g -lt 6; $g++) {
            for ($b = 0; $b -lt 6; $b++) {
                ...
                $foregroundColor = if ($colorIndex -lt 100) { 255 } else { 0 }
                $colorText = Set-Ansi256Color -Text (" {0:D3} " -f $colorIndex) -BackgroundColor $colorIndex -ForegroundColor $foregroundColor
                Write-Host $colorText -NoNewline
            }
    ...
    if ($ShowGrayscale) {
        ...
        for ($i = 232; $i -lt 256; $i++) {
            $foregroundColor = if ($i -lt 244) { 255 } else { 0 }
            $colorText = Set-Ansi256Color -Text (" {0:D3} " -f $i) -BackgroundColor $i -ForegroundColor $foregroundColor
            Write-Host $colorText -NoNewline
